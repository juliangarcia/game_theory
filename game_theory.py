...
class BimatrixGame:
    ...
    def find_unique_equilibrium(self, atol=10e-3):
        ...
        if (len(candidates) > 3):
            raise ValueError("Degenerate case, too many pure equilibria")
        ...
        if ((0.0, 1.0) in candidates):
            ...
            except NoRiskDominanceSolvable:
    ...
    def expected_payoff(self, profile):
        ...
        pi_2 =  p*q*self.a2+p*(1.0-q)*self.b2+(1.0-p)*q*self.c2+(1.0-p)*(1.0-q)*self.d2
        return (pi_1, pi_2)
    ...
    def reduce_by_risk_dominance(self, a, b):
        ...
        if (a < b):
            return(1.0, 1.0)
        raise NoRiskDominanceSolvable("No risk dominance, no payoff dominance: " + str(self))    


    def plot_payoff_space(self, size_x= 5, size_y=5, grid_on = True):
        maximum_payoff = max(self.a1,self.a2,self.b1,self.b2,self.c1,self.c2,self.d1,self.d2)
        minimum_payoff = min(self.a1,self.a2,self.b1,self.b2,self.c1,self.c2,self.d1,self.d2)
